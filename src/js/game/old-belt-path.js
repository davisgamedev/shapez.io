...
export class BeltPath extends BasicSerializableObject {
    ...
    updateExperimental() {
        ...
        for (lastItemProcessed = this.items.length - 1; lastItemProcessed >= 0; --lastItemProcessed) {
            ...
            //     0,
            //     Math.min(remainingDistributeDistance, nextDistanceAndItem[_nextDistance] - minimumSpacing)
            // );

            let clampedProgress;
            let diff = nextDistanceAndItem[_nextDistance] - minimumSpacing;
            ...
            this.spacingToFirstItem += clampedProgress;
                ...
                if the distance is greater, we take the remaining velocity
                if the velocity is less than 0, we take 0

                 remainingVelocity -= clampedProgress
                 therefore if remaining velocity, get 0
                 therfore if 0, get remaining velocity (very small)

             */

            // note check end for remaining edge cases
            if (diff < 0 || remainingVelocity < 0) {
                /**
                 * ! alskdjalskdjlaskjd
                 */
            }

            // remaining velocity can never be negative, it starts at belt speed

            if (diff > remainingVelocity) {
                if (remainingVelocity < 0) {
                    // remainingVelocity is 0
                    if (isFirstItemProcessed && nextDistanceAndItem[_nextDistance] < 1e-7) {
                        ...
                        if (this.tryHandOverItem(nextDistanceAndItem[_item], beltSpeed)) {
                            ...
                            if (this.numCompressedItemsAfterFirstItem > 0)
                                this.numCompressedItemsAfterFirstItem--;
                            else this.numCompressedItemsAfterFirstItem = 0;
                        }
                    }
                } else {
                    // non negative, small
                    ...
                    if (isFirstItemProcessed && nextDistanceAndItem[_nextDistance] < 1e-7) {
                        ...
                        const excessVelocity = beltSpeed - remainingVelocity;
                        if (this.tryHandOverItem(nextDistanceAndItem[_item], excessVelocity)) {
                            ...
                            if (this.numCompressedItemsAfterFirstItem > 0) {
                                ...
                                if (itemBehind) {
                                    // then remaining velocity
                                    if (itemBehind[_nextDistance] > remainingVelocity) {
                                        this.spacingToFirstItem += remainingVelocity;
                                        itemBehind[_nextDistance] -= remainingVelocity;
                                        remainingVelocity = 0; // consume velocity
                                    } else if (itemBehind[_nextDistance] > 0) {
                                        // use item behind, it's greater than 0 but less than remain
                                        this.spacingToFirstItem += itemBehind[_nextDistance];
                                        remainingVelocity = itemBehind[_nextDistance] = 0;
                                    }
                                }
                                this.numCompressedItemsAfterFirstItem--;
                            } else if (this.numCompressedItemsAfterFirstItem < 0)
                                this.numCompressedItemsAfterFirstItem = 0;
                        }
                        //}
                    }
                    // dif < remainingVelocity
                    else {
                        if (diff < 0) {
                            if (isFirstItemProcessed && nextDistanceAndItem[_nextDistance] < 1e-7) {
                                isFirstItemProcessed = false;
                            ...
                            // diff > 0, < remaing
                            remainingVelocity -= diff;
                            nextDistanceAndItem[_nextDistance] -= diff;
                            this.spacingToFirstItem += diff;
                            ...
                            if (isFirstItemProcessed && nextDistanceAndItem[_nextDistance] < 1e-7) {
                                ...
                                const excessVelocity = beltSpeed - diff;
